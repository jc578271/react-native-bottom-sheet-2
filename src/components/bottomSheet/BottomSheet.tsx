...
const BottomSheetComponent = forwardRef<BottomSheet, BottomSheetProps>(function BottomSheet(props, ref) {
  ...
  // keyboard
  const {state: animatedKeyboardState, textInputNodesRef} = useAnimatedKeyboard();
  const userReduceMotionSetting = useReducedMotion();
  const reduceMotion = useMemo(() => {
    return !_providedOverrideReduceMotion || _providedOverrideReduceMotion === ReduceMotion.System
  ...
  const animatedSheetState = useDerivedValue(() => {
    ...
    const extendedPositionWithKeyboard = Math.max(
      0,
      containerHeight - (animatedSheetHeight.value + keyboardHeightInContainer)
    );
  ...
  const animatedIndex = useDerivedValue(() => {
    ...
    const {status: animationStatus, source: animationSource, nextIndex, nextPosition} = animatedAnimationState.get();
    /**
  ...
  const animateToPosition: AnimateToPositionType = useCallback(
    function animateToPosition(
      ...
      // the same position
      const {status: animationStatus, nextPosition} = animatedAnimationState.get();
      ...
      const {detents, closedDetentPosition, highestDetentPosition} = animatedDetentsState.get();
  ...
  const getEvaluatedPosition = useCallback(
    function getEvaluatedPosition(source: ANIMATION_SOURCE) {
      ...
      const {detents, highestDetentPosition, closedDetentPosition} = animatedDetentsState.get();
      ...
      const {status, nextIndex, nextPosition} = animatedAnimationState.get();
  ...
  const evaluatePosition = useCallback(
    function evaluatePosition(source: ANIMATION_SOURCE, animationConfigs?: WithSpringConfig | WithTimingConfig) {
      'worklet';
      const {status: animationStatus, nextIndex, isForcedClosing} = animatedAnimationState.get();
  ...
  const handleSnapToIndex = useStableCallback(function handleSnapToIndex(
     ...
     */
    const {nextPosition, nextIndex, isForcedClosing} = animatedAnimationState.get();
  ...
  useAnimatedReaction(
    ...
    (result, _previousResult) => {
      ...
      const {status, height, easing, duration, target} = animatedKeyboardState.get();
      ...
      const {status: animationStatus, source: animationSource} = animatedAnimationState.get();
      /**
  ...
  return (
    <BottomSheetProvider value={externalContextVariables}>
      <BottomSheetInternalProvider value={internalContextVariables}>
        <BottomSheetGestureHandlersProvider gestureEventsHandlersHook={gestureEventsHandlersHook}>
          {BackdropComponent ? (
            <BackdropComponent
          ...
          >
            <BottomSheetBody style={style}>
              ...
              >
                {children}
                {footerComponent ? <BottomSheetFooterContainer footerComponent={footerComponent} /> : null}
              </BottomSheetContent>
              {handleComponent !== null ? (
